    ...
    if vtable.bases.iter().any(|base| base.type_name.contains(class_name)) {
      ...
      for base_class_info in &vtable.bases {
        ...
        if !base_infos.contains_key(&base_class) {
          let base_info = vtables.iter().find(|vtable| vtable.type_name.eq(&base_class));
      ...
      writeln!(entity_classes_file, "{:<50} [{}]", vtable.type_name, vtable.methods.len())?;

      let mut file = File::create(format!("{}/entity_diff/{}/{}.txt", dump_dir, binary_name, vtable.type_name))?;
      writeln!(file, "{:<15} VS {:>15}", base_info.type_name, vtable.type_name)?;
